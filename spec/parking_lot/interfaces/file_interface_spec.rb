...
RSpec.describe ParkingLot::Interfaces::FileInterface do
  context "when file doesn't exist" do
    let(:file_interface) { described_class.new 'non_existant_file.txt' }

    it 'raises an error' do
      expect { file_interface.process }.to raise_error(Errno::ENOENT)
    end
  end
  ...
  context 'when file exists' do
    ...
    # TIL:
    # using the matcher below, we don't need to stub out stdout
    # expect do
    #   # code
    # end.to output(expected_out).to_stdout

    describe '#process' do
      let(:parking_lot) { instance_double(ParkingLot) }
      let(:file) { instance_double(File, readlines: commands.split("\n")) }
      let(:file_interface) { described_class.new 'file_inputs.txt' }
